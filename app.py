...
def index():
    ...
    top_category = pd.crosstab(index=df2['Category'], columns='Jumlah').sort_values(by='Jumlah', ascending=False).reset_index().head()
...
## Buatlah sebuah plot yang menampilkan insight di dalam data 
    df2[['Type', 'Content Rating']] = df2[['Type', 'Content Rating']].astype('category')

    pie = pd.crosstab(index=df2['Content Rating'], columns=df2['Type'], values=df2['Rating'], aggfunc='mean')
    pie.round(1)
    pie = pie.melt().round(1).ffill().bfill()

    fig, ax = plt.subplots(figsize=(12, 12), subplot_kw=dict(aspect="equal"))
    recipe = pie['Type']
    data = pie['value']
    informations = [x.split()[-1] for x in recipe]
    colors= ["lightpink", "sandybrown", "goldenrod", "darkkhaki", "limegreen", "mediumseagreen", "lightseagreen", "darkturquoise", "lightskyblue", "plum", "violet", "orchid"]


    def func(pct, allvals):
        absolute = int(pct/100.*np.sum(allvals))
        return "{:.1f}%\n({:d} star)".format(pct, absolute)


    wedges, texts, autotexts = ax.pie(data, colors=colors, autopct=lambda pct: func(pct, data), textprops=dict(color="w"))
    ax.legend(wedges, informations, title="Informations", loc="center left", bbox_to_anchor=(1, 0, 0.5, 1))
